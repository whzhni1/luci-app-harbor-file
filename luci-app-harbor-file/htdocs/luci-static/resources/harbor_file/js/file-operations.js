/* HarborFile file-operations.js - file manager business logic (navigation, transfer, archive, clipboard...) */
(function (HF) {
    ...
    HF.thumbnail_url = function thumbnail_url(item) {
        return HF.api.thumbnail + '?path=' + encodeURIComponent(item.path) +
    ...
    HF.poll_archive_task = function poll_archive_task(task_id) {
        ...
        HF.Util.ajax({
            ...
            success: function(res) {
                ...
                if (HF.state.archive_task && HF.state.archive_task.done) {
                    /* The archive window already shows the outcome; no toast. */
    ...
    /* A required archive tool (zip/unzip/tar/gzip) is missing: reuse the
     * GraphicsMagick pattern — open the task window with an install button. */
    ...
    HF.can_move_to_path = function can_move_to_path(path) {
    ...
    HF.apply_selection_classes = function apply_selection_classes() {
        ...
        for (var i = 0; i < nodes.length; i++) {
            nodes[i].classList.toggle('selected', !!selected[nodes[i].getAttribute('data-path')]);
        }
    }
    ...
    HF.bind_item_hover = function bind_item_hover(node, item) {
        ...
        node.addEventListener('focus', function() {
            ...
            HF.show_item_hover_card(item, {
                clientX: rect.left + Math.min(rect.width, 24),
                clientY: rect.bottom
            });
        });
        node.addEventListener('blur', HF.hide_item_hover_card);
    ...
    HF.update_marquee_selection = function update_marquee_selection(event) {
        ...
        var rect = HF.selection_rect(HF.state.selection_start_x, HF.state.selection_start_y, event.clientX, event.clientY);
    ...
    /* ---- On-demand file-type detection for unknown extensions ----
     * Files with an unknown/no extension show a generic icon and are sniffed
     * (UTF-8 check on the backend) only when the user actually acts on them:
     * right-clicking (to decide whether "Open" lights up) or opening them.
     * Listing a directory never reads file contents. Results are cached for
     * the session and concurrent requests for the same file are coalesced. */
    ...
    HF.create_item = function create_item(item, kind) {
        ...
        node.addEventListener('contextmenu', function(event) {
            ...
            /* 'Open' stays disabled until the file type is known. On
             * right-click we sniff this single file so the button can light
             * up as soon as the type is resolved. */
            HF.set_button_state('fm_menu_open', is_multi || (item.type !== 'directory' && item.preview === 'none'), is_multi ? HF.labels.multiple_items : '');
            if (!is_multi && item.type !== 'directory' && item.preview === 'none') {
                HF.request_detect_type(item.path, function(type) {
                    item.preview = type === 'text' ? 'text' : 'binary';
                    HF.set_button_state('fm_menu_open', false, '');
                });
            }
        ...
        if ('ontouchstart' in window) {
            ...
            node.addEventListener('touchstart', function(e) {
                ...
                var main = node.querySelector('.fm-item-main');
                if (main) {
    ...
    HF.set_upload_panel = function set_upload_panel(title, detail, percent, class_name, uploaded_bytes, total_bytes) {
        ...
        if (class_name === 'success' || class_name === 'error') {
            /* Keep the result visible for a moment, then close the window. */
    ...
    HF.open_name_dialog = function open_name_dialog(action, item) {
        ...
        document.getElementById('fm_name_label').textContent =
            is_rename ? HF.labels.new_name : (is_create_file ? HF.labels.file_name : HF.labels.folder_name);
        document.getElementById('fm_name_confirm').textContent = is_rename ? HF.labels.save : HF.labels.create;
        var input = document.getElementById('fm_name_input');
        input.value = is_rename && item ? item.name : '';
    ...
    HF.trigger_download = function trigger_download(item) {
        ...
        link.href = HF.api.download + '?path=' + encodeURIComponent(item.path) + '&v=' + encodeURIComponent(item.mtime || 0);
    ...
    HF.showPasteConflictDialog = function showPasteConflictDialog(conflicts, action, items, target_path) {
        ...
        var cancelBtn = document.getElementById('fm_overwrite_cancel');
    ...
    HF.hide_context_menus = function hide_context_menus() {
        HF.hide_item_hover_card();
        document.getElementById('fm_item_menu').style.display = 'none';
        document.getElementById('fm_blank_menu').style.display = 'none';
        document.getElementById('fm_view_menu').style.display = 'none';
        document.getElementById('fm_sort_menu').style.display = 'none';
    }
    ...
    HF.show_context_menu = function show_context_menu(id, x, y) {
        HF.hide_context_menus();
    ...
    HF.poll_package_install = function poll_package_install(task_id) {
        ...
        HF.Util.ajax({
            ...
            success: function(res) {
                ...
                if (HF.state.install_task && HF.state.install_task.done) {
                    /* The install window already shows the outcome; no toast. */
                }
            },
            error: function(xhr) {
                HF.set_error_status(HF.upload_error_message(xhr, HF.labels.request_failed));
    ...
    HF.poll_thumbnail_generation = function poll_thumbnail_generation(task_id) {
        ...
        HF.Util.ajax({
            ...
            success: function(res) {
                ...
                if (HF.state.thumbnail_task && HF.state.thumbnail_task.done) {
                    /* The thumbnail window already shows the outcome; no toast. */
    ...
    HF.open_item = function open_item(item) {
        HF.hide_context_menus();
        if (!item) return;
    ...
    HF.append_crumb = function append_crumb(container, label, on_click) {
        var button = document.createElement('button');
        button.type = 'button';
    ...
    HF.render_breadcrumb = function render_breadcrumb() {
        var address = document.getElementById('fm_address');
        HF.clear_node(address);

        var crumbContainer = document.createElement('span');
        crumbContainer.className = 'fm-breadcrumb-container';
        var input = document.createElement('input');
        input.type = 'text';
        input.id = 'fm_address_input';
        input.className = 'fm-address-input';
        input.style.display = 'none';
        address.appendChild(crumbContainer);
        address.appendChild(input);

        function addCrumb(container, label, on_click) {
            var button = document.createElement('button');
            button.type = 'button';
            button.className = 'fm-crumb';
            button.textContent = label;
        ...
        if (HF.state.current.kind === 'quick_access') {
            addCrumb(crumbContainer, HF.labels.quick_access, function() {});
            input.style.display = 'none';
            return;
        }

        addCrumb(crumbContainer, HF.labels.this_pc, function() { HF.show_virtual('this_pc', true); });
        ...
        var parts = HF.state.current.path.split('/').filter(function(part) { return part !== ''; });
        var current_path = '';
        if (!parts.length) {
            addCrumb(crumbContainer, HF.labels.system_disk, function() { HF.open_path('/', true); });
            input.value = '/';
            input.style.display = 'none';
            crumbContainer.style.display = 'inline-flex';
            return;
        }
        parts.forEach(function(part) {
            current_path += '/' + part;
            var target = current_path;
            addCrumb(crumbContainer, part, function() { HF.open_path(target, true); });
        });

        input.value = HF.state.current.path;
        input.style.display = 'none';
        crumbContainer.style.display = 'inline-flex';
    ...
    HF.enter_address_edit = function enter_address_edit() {
        if (HF.state.address_editing) return;
        var input = document.getElementById('fm_address_input');
        var container = document.querySelector('.fm-breadcrumb-container');
