/* HarborFile util.js - config, state, networking & formatting helpers (loaded first) */
(function (HF) {
    ...
    HF.state = {
        navigation: { quick_access: [], folders: [], drives: [] },
        preferences: {
            view_mode: 1,
    ...
    /* ---- haptic feedback (guarded against Chrome's user-activation rule) ----
     * Chrome blocks navigator.vibrate outside a user gesture and logs a noisy
     * console warning ("Blocked call to navigator.vibrate..."). Only vibrate
     * while a gesture is actually active; ignore otherwise. */
    HF.safe_vibrate = function safe_vibrate(ms) {
    ...
    /* ---- Window-based notices (Windows-style) ----
     * Errors and warnings stay open until dismissed; success notices close
     * themselves. Every notice is a real desktop window: it opens centred,
     * clamps to the viewport and can be dragged / closed like any window. */
    ...
    /* Window-based confirm dialog. Resolves true (confirm) / false (cancel). */
    HF.confirm_window = function confirm_window(options) {
        ...
        return new Promise(function(resolve) {
            ...
            /* Modal: a full-screen overlay blocks every other window until the
             * user makes a choice, and the confirm window stays on top. */
    ...
    HF.set_status = function set_status(message, type, duration) {
        ...
        if (type === 'error' || type === 'warning') {
            auto = 0; /* stays open until the user closes it */
        } else {
    ...
    // ---- native AJAX (replaces jQuery $.ajax / $.trim) ----
    HF.Util = HF.Util || {};
    ...
    // ---- small undo/redo history helper (coalesced snapshots) ----
        ...
        return {
            capture: function (snapshot) {
                ...
                if (coalesce > 0 && now - lastCapture < coalesce && undoStack.length) {
                    return; /* coalesce rapid typing into one step */
                }
