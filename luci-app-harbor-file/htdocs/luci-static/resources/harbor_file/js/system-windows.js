/* HarborFile system-windows.js - properties/settings/terminal/modal system windows */
(function (HF) {
    ...
    HF.resolve_terminal_url = function resolve_terminal_url(info) {
        ...
        if (raw) {
            /* Same-origin relative path (reverse-proxy mount): always safe. */
            if (raw.charAt(0) === '/') {
                return window.location.origin + raw;
            }
            if (/^https?:\/\//i.test(raw)) {
                /* Full override (ttyd url_override). It is meant for remote
                 * access, so honour it on an https page. On an http page
                 * (local LAN) prefer the router's own ttyd instance. */
                if (https_page) {
                    return raw;
                }
            }
        ...
        /* No usable override: fall back to the router's own ttyd. An https
         * LuCI page can never embed an http:// iframe (mixed content), so
         * mirror the page scheme here. */
    ...
    HF.open_modal = function open_modal(id) {
        ...
        var display_mode = (id === 'fm_terminal_dialog') ? 'flex' : 'block';
        /* Dialogs are non-modal desktop windows now; the reusable manager also
         * adds caption controls, drag bounds, resizing and a taskbar entry. */
    ...
    HF.attach_terminal_frame = function attach_terminal_frame(url) {
        ...
        if (/^https:/i.test(url)) {
            /* No fetch pre-flight for HTTPS. Give the iframe a grace period,
             * then surface a clear error if it never finished loading. */
        ...
        HF.Util.ajax({
            ...
            success: function(res) {
                ...
                if (window.location.protocol === 'https:' && /^http:/i.test(url)) {
                    /* An http:// terminal inside an https:// page is always
                     * blocked by the browser (mixed content). Surface a
                     * clear error instead of waiting for the frame to fail. */
                ...
                if (/^https:/i.test(url)) {
                    /* A self-signed certificate makes the fetch pre-flight
                     * unreliable, so attach the frame directly and let it load
                     * (or time out with a clear error). */
    ...
    HF.close_modal = function close_modal() {
        if (HF.state.nginx_installing &&
                document.getElementById('fm_nginx_install_dialog').style.display === 'block') {
            return;
        }
        ...
        var cancel_pending = document.getElementById('fm_overwrite_dialog').style.display === 'block' && HF.state.pending_upload;
