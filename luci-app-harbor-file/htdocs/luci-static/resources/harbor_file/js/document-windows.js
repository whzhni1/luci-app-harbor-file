/* HarborFile document-windows.js - text/hex/image/pdf/video document windows */
(function (HF) {
    ...
    HF.wm_should_auto_indent = function wm_should_auto_indent(path) {
        var name = String(path || '').split('/').pop();
        if (['.bashrc', '.profile', '.bash_profile', '.zshrc', '.vimrc'].indexOf(name) >= 0) {
            return true;
        }
        ...
        var code_ext = ['sh', 'bash', 'zsh', 'py', 'lua', 'js', 'ts', 'jsx', 'tsx', 'c', 'h',
            'cpp', 'cc', 'hpp', 'java', 'go', 'rs', 'php', 'rb', 'pl', 'json', 'yaml', 'yml',
            'conf', 'ini', 'cfg', 'toml', 'css', 'scss', 'sql', 'awk', 'mjs'];
        return code_ext.indexOf(ext) >= 0;
    }
    ...
    HF.wm_open_image_window = function wm_open_image_window(item) {
        ...
        image.src = HF.api.image + '?path=' + encodeURIComponent(item.path) + '&v=' + encodeURIComponent(item.mtime || 0);
        return record;
    }
    ...
    HF.wm_open_pdf_window = function wm_open_pdf_window(item) {
        ...
        frame.src = HF.api.pdf + '?path=' + encodeURIComponent(item.path) + '&v=' + encodeURIComponent(item.mtime || 0);
        return record;
    }
    ...
    HF.wm_open_video_window = function wm_open_video_window(item) {
        ...
        function start_playback() {
            ...
            video.src = HF.api.video + '?path=' + encodeURIComponent(item.path) + '&v=' +
                encodeURIComponent(item.mtime || 0) + '&stream=4';
            video.load();
        }
        HF.Util.ajax({
            url: HF.api.video_check,
            type: 'GET',
            data: { path: item.path },
            dataType: 'json',
            ...
            success: function(res) {
                ...
                var info = res && res.data ? res.data : null;
                if (info && !info.playable) {
                    error.textContent = HF.labels.video_no_range;
                    error.style.display = 'block';
                    return;
                }
                if (info && !info.range_supported) {
                    hint.style.display = 'block';
                }
                start_playback();
            },
            error: start_playback
        });
        return record;
    }
    ...
    /* Full-memory editor core: data model separate from the tiny virtual DOM. */
    HF.wm_json_request = function wm_json_request(url, data, method) {
            ...
            HF.Util.ajax({
                ...
                success: function(res) {
                    if (res && res.code === 0) {
                        ...
                        reject((res && res.message) || HF.labels.request_failed);
                    }
                },
                error: function(xhr) { reject(HF.upload_error_message(xhr, HF.labels.request_failed)); }
            });
        });
    }

    HF.wm_base64_to_bytes = function wm_base64_to_bytes(value) {
    ...
    HF.wm_refresh_search_counter = function wm_refresh_search_counter(search, model) {
        search.setStatus(model.current, model.matches.length);
    }

    HF.wm_measure_text_width = function wm_measure_text_width(element, text) {
    ...
    HF.wm_read_editor_file = function wm_read_editor_file(item) {
        return HF.wm_json_request(HF.api.read_editor_file, { path: item.path }).then(function(data) {
            return { bytes: HF.wm_base64_to_bytes(data.content_base64 || ''), size: Number(data.size || 0), mtime: Number(data.mtime || 0) };
        });
    }
        ...
        return {
            ...
            setCursor: function(line, column) {
                if (line === null || line === undefined || column === null || column === undefined) {
                    return; /* keep the last position visible (the node is clickable) */
                }
    ...
    HF.wm_create_search = function wm_create_search(record, content, options) {
        ...
        var close = HF.Util.createElement('button', 'fm-editor-search-close', '×');
        [previous, next, close].forEach(function(button) { button.type = 'button'; });
        row.appendChild(input); row.appendChild(count); row.appendChild(previous); row.appendChild(next); row.appendChild(case_label); row.appendChild(close);
        ...
        /* Replace / Replace all are buttons now: ↑↓ navigates between matches,
        ...
        input.addEventListener('keydown', function(event) { if (event.key === 'Enter') { event.preventDefault(); execute(1); } });
        ...
        search_button.addEventListener('click', function(event) {
            ...
            search_button.classList.toggle('is-open');
            shell.classList.toggle('show', search_button.classList.contains('is-open'));
    ...
    HF.wm_open_memory_text_window = function wm_open_memory_text_window(item) {
        ...
        /* Measure how many visual rows each line occupies, then render only
         * the lines currently inside the viewport (virtualised gutter) so the
         * wrap indicators never cost a full-file DOM render. */
        function compute_gutter(text) {
            gutter_line_height();
            var lines = text.split('\n');
            var n = lines.length;
            var rows;
            if (!session.wrap || n > 2000 || gutter.offsetParent === null) {
                /* Fast path: no wrap, huge file, or the gutter is hidden. */
                rows = new Array(n);
                for (var i = 0; i < n; i++) { rows[i] = 1; }
            } else {
                if (!gutter_mirror) {
                ...
                gutter_mirror.style.width = editor.clientWidth + 'px';
                gutter_mirror.textContent = '';
                var mirrors = new Array(n);
                var frag = document.createDocumentFragment();
                for (var m = 0; m < n; m++) {
                    var d = document.createElement('div');
                    d.textContent = lines[m] === '' ? ' ' : lines[m];
                    mirrors[m] = d;
                    frag.appendChild(d);
                }
                gutter_mirror.appendChild(frag);
                rows = new Array(n);
                for (var r = 0; r < n; r++) {
                    rows[r] = Math.max(1, Math.round(mirrors[r].getBoundingClientRect().height / gutter_line_h));
                }
                gutter_mirror.textContent = '';
            }
            gutter_rows = rows;
        ...
        function save() {
            ...
            HF.wm_upload_editor_blob(item, new Blob([bytes], { type: 'application/octet-stream' }), function(loaded, total) {
                ...
                tip_text(HF.tr('Saving ') + Math.round(loaded * 100 / total) + '%');
            }).then(function(data) {
        ...
        var footer = HF.wm_create_memory_footer({
            ...
            /* Clicking the "line/column" readout toggles the line numbers and
             * persists the choice to UCI. */
        ...
        /* Tracks the caret offset for undo/redo snapshots. */
        var last_cursor = null;

        /* Live cursor line/column readout in the footer. */
        ...
        editor.addEventListener('input', function() {
            ...
            /* The input event fires before the post-edit selectionchange, so
             * last_cursor is still the pre-edit caret position: undo/redo can
             * restore the caret to exactly where the edit happened. */
            history.capture({
                text: session.text,
                cursor: last_cursor,
                scrollTop: editor.scrollTop,
        ...
        var auto_indent = HF.state.preferences.editor_auto_indent === 1 && HF.wm_should_auto_indent(item.path);
        editor.addEventListener('keydown', function(event) {
            if (event.key === 'Enter' && !event.shiftKey && auto_indent && !session.readonly) {
                event.preventDefault();
                var offset = selection_offset();
                ...
                var trimmed = current_line.replace(/[ \t]+$/, '');
                var extra = /[{\[(]\s*$/.test(trimmed) ? '    ' : '';
    ...
    HF.wm_open_memory_hex_window = function wm_open_memory_hex_window(item) {
        var session = { item: item, bytes: new Uint8Array(0), original_bytes: new Uint8Array(0), dirty: false, readonly: !HF.can_modify_system_path(item.path), saving: false, matches: [], current: -1, query: '', caseSensitive: false, selected: -1, nibble: false, pane: 'hex', rowHeight: 26, renderTimer: null };
        var history = HF.Util.createHistory(100, 0); /* hex: byte-level undo, never coalesce */
        var content = HF.wm_document_column();
        ...
        /* Hidden input that summons the mobile soft keyboard. The visible ASCII
         * cells are <span> elements edited via keydown; on touch devices this
         * input captures the IME and routes characters into the byte buffer. */
        ...
        ime_input.addEventListener('input', function() {
            ...
            if (offset < 0 || offset >= session.bytes.length) { ime_prepare(); return; }
            ...
            if (value.length > 1) {
                ...
                if (pane === 'hex') {
                    if (/^[0-9a-fA-F]$/.test(ch)) {
                        ...
                        if (!session.nibble) {
                            capture_change();
                            session.bytes[offset] = digit << 4;
                            session.nibble = true;
                        } else {
                            session.bytes[offset] = (session.bytes[offset] & 0xF0) | digit;
                    ...
                    capture_change();
                    session.bytes[offset] = ch.charCodeAt(0) & 255;
                    refresh_dirty();
                    sync_pair(offset);
                    refresh();
                    history_buttons.refresh(history.canUndo(), history.canRedo());
        ...
        function refresh_dirty() {
            session.dirty = !bytes_equal(session.bytes, session.original_bytes);
        }
        function recovery_save() {
            if (session.dirty) {
                HF.Recovery.saveBuffer('hex', item.path, {
                    bytes: HF.Recovery.bytesToBase64(session.bytes),
        ...
        function ascii_char(offset) {
            var code = session.bytes[offset];
            return code >= 32 && code <= 126 ? String.fromCharCode(code) : '·';
        }
        function hex_char(offset) {
            return ('0' + session.bytes[offset].toString(16)).slice(-2).toUpperCase();
        }
        ...
        function move_selection(delta) {
            ...
            if (next < 0 || next >= session.bytes.length) return;
            select(next);
            reveal_offset(next);
        ...
        function handle_ascii_key(event) {
            ...
            var key = event.key;
            var offset = session.selected;
            if (offset < 0 || offset >= session.bytes.length) return;
            if (key && key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
                event.preventDefault();
                capture_change();
                session.bytes[offset] = key.charCodeAt(0) & 255;
                refresh_dirty();
                sync_pair(offset);
                refresh();
                history_buttons.refresh(history.canUndo(), history.canRedo());
                ...
                event.preventDefault();
                var down = offset + 16;
                if (down < session.bytes.length) { select(down); reveal_offset(down); if (!('ontouchstart' in window)) focus_cell(down); }
            }
        }
        ...
        function paint() {
            var rows = Math.ceil(session.bytes.length / 16);
            ...
            for (var row_index = start; row_index < end; row_index++) {
                ...
                for (var column = 0; column < 16; column++) {
                    (function(byte_offset) {
                        ...
                        hex_cell.tabIndex = 0;
                        if (byte_offset >= session.bytes.length) {
        ...
        function hex_string() { var out = new Array(session.bytes.length); for (var i = 0; i < session.bytes.length; i++) out[i] = ('0' + session.bytes[i].toString(16)).slice(-2).toUpperCase(); return out.join(''); }
        function find(query, regex) {
            var source = hex_string(), compact = String(query || '').replace(/\s+/g, '').toUpperCase(); if (!compact) return [];
            var expression = regex ? compact.replace(/\?\?/g, '[0-9A-F]{2}') : HF.wm_escape_regex(compact); var regexp; try { regexp = new RegExp(expression, 'g'); } catch (error) { return []; }
            var matches = [], match; while ((match = regexp.exec(source))) { if (match.index % 2 === 0 && match[0].length % 2 === 0) matches.push({ index: match.index / 2, length: match[0].length / 2 }); if (!match[0].length) regexp.lastIndex++; } return matches;
        }
        function clear() { HF.wm_reset_search_state(session); HF.wm_refresh_search_counter(search, session); paint(); }
        function navigate(index) {
            if (!session.matches.length) return clear(); session.current = (index + session.matches.length) % session.matches.length; var match = session.matches[session.current]; session.selected = match.index; HF.wm_refresh_search_counter(search, session);
        ...
        function search_hex(query, regex, case_sensitive, direction) {
            session.caseSensitive = !!case_sensitive; var current = HF.wm_commit_search(session, query, find(query, regex), direction); if (current >= 0) navigate(current); else { HF.wm_refresh_search_counter(search, session); paint(); }
        }
        function save() {
            if (session.readonly || session.saving || !session.dirty) return; session.saving = true; refresh();
            HF.wm_upload_editor_blob(item, new Blob([session.bytes], { type: 'application/octet-stream' }), function(loaded, total) { progress.set(loaded, total); tip_text(HF.tr('Saving ') + Math.round(loaded * 100 / total) + '%'); }).then(function(data) {
                session.saving = false; session.original_bytes = session.bytes.slice(); session.dirty = false; HF.Recovery.clearBuffer(item.path); history_buttons.refresh(history.canUndo(), history.canRedo()); progress.hide(); item.size = Number(data.size || session.bytes.length); item.mtime = Number(data.mtime || item.mtime || 0); item.display_size = HF.format_size(item.size); item.display_mtime = HF.format_time(item.mtime || 0); tip_text(''); refresh();
            }).catch(function(error) { session.saving = false; progress.hide(); refresh(); tip_text(error || HF.labels.save_failed); });
        }
        ...
        var record = HF.window_manager.create({ title: item.path || item.name, icon: item.icon_name || 'binary', className: 'fm-window-binary', content: content, width: 840, height: 590, minWidth: 390, minHeight: 270, beforeClose: function() { if (session.saving) { return false; } if (!session.dirty) { return true; } return HF.confirm_window({ title: HF.tr('Close'), message: HF.labels.discard_text_changes }); }, onClose: function() { HF.Recovery.clearBuffer(item.path); } });
        var footer = HF.wm_create_memory_footer({ size: function() { return session.bytes.length; }, get saving() { return session.saving; }, get dirty() { return session.dirty; }, get readonly() { return session.readonly; } }, save, function() { HF.window_manager.close(record); });
        content.appendChild(footer.element);
        var history_buttons = HF.wm_add_history_buttons(record, undo_edit, redo_edit);
        history_buttons.refresh(history.canUndo(), history.canRedo());
        ...
        var search = HF.wm_create_search(record, content, { placeholder: HF.tr('Enter hex bytes, e.g. DE AD BE EF'), replacePlaceholder: HF.tr('Replace hex value'), hex: true, clear: clear, search: search_hex, replaceStep: function(value, all, direction) {
            var replacement = HF.wm_parse_hex_bytes(value); if (!replacement || !session.matches.length) return; var current = session.current >= 0 ? session.current : (direction < 0 ? session.matches.length - 1 : 0); var list = all ? session.matches.slice() : [session.matches[current]]; if (!list.every(function(match) { return match.length === replacement.length; })) return tip_text(HF.tr('Hex replacement must match the match length')); history.capture(edit_state()); list.sort(function(a, b) { return a.index - b.index; }); list.forEach(function(match) { session.bytes.set(replacement, match.index); }); refresh_dirty(); HF.wm_commit_search(session, session.query, find(session.query, false), 0); if (session.matches.length) { session.current = all ? 0 : (direction < 0 ? (current - 1 + session.matches.length) % session.matches.length : current % session.matches.length); navigate(session.current); } else { HF.wm_refresh_search_counter(search, session); paint(); } refresh(); history_buttons.refresh(history.canUndo(), history.canRedo()); recovery_save();
        } });
        ...
        function start_load() {
            ...
            HF.wm_read_editor_file(item).then(function(data) {
                ...
                session.original_bytes = data.bytes.slice();
                refresh_dirty();
                paint();
                if (session.selected < 0 && session.bytes.length) {
                    session.selected = 0;
                }
                update_cursor_hex();
                footer.refresh();
        ...
        if (HF.Recovery.hasBuffer(item.path) === 'hex') {
            HF.wm_create_recover_banner(content, function() {
                var rec = HF.Recovery.getBuffer(item.path);
                var recovered = null;
                if (rec && rec.payload && rec.payload.bytes) {
                    recovered = HF.Recovery.base64ToBytes(rec.payload.bytes);
                }
                HF.wm_read_editor_file(item).then(function(data) {
                    session.original_bytes = data.bytes.slice();
                    session.bytes = recovered || data.bytes;
                    ...
                    session.bytes = recovered || new Uint8Array(0);
                    session.original_bytes = new Uint8Array(0);
                    refresh_dirty();
                    paint();
                    update_cursor_hex();
                    var payload = rec && rec.payload || {};
    ...
    /* File openings now create independent document windows instead of reusing
     * singleton dialogs.  Existing HF.open_item()/context menu paths call these
     * function bindings dynamically, so no duplicated event wiring is needed. */
    ...
    HF.open_binary = function(item) {
        return HF.wm_open_binary_window(item);
    };

    // ---- public DocumentWindows API ----
    HF.DocumentWindows = {
