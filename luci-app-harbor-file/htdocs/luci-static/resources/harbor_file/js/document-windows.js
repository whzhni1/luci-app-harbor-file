...
(function (HF) {
    ...
    HF.wm_open_byte_source = function wm_open_byte_source(item) {
        return new HarborByteSource(item.path, Number(item.size || 0));
    }
    ...
    HF.wm_open_memory_text_window = function wm_open_memory_text_window(item) {
        ...
        function start_load() {
            ...
            HF.wm_read_editor_file(item).then(function(data) {
                ...
                tip_text('');
                if (session.readonly) {
                    tip_text(HF.labels.system_folder_blocked, 'warning');
                }
    ...
    HF.wm_open_memory_hex_window = function wm_open_memory_hex_window(item) {
        ...
        function save() {
            ...
            HF.wm_upload_editor_blob(item, new Blob([session.bytes], { type: 'application/octet-stream' }), function(loaded, total) { progress.set(loaded, total); }).then(function(data) {
