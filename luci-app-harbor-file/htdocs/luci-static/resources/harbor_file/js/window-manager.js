/* HarborFile window-manager.js - window/taskbar behaviour only (no file/editor knowledge) */
(function (HF) {
    HF.window_manager = (function() {
        ...
        DesktopWindowManager.prototype.bind_taskbar_reveal = function() {
            var manager = this;
            /* Capture scroll sees #fm_content as well as document scrolling, but
             * deliberately ignores editor, search and modal inner scrollers. */
            ...
            var operation_handler = function() {
                /* A completed click/tap, rather than pointer-down, avoids
                 * revealing the taskbar during a title-bar drag or resize. */
            ...
            if (window.visualViewport) {
                window.visualViewport.addEventListener('resize', function() {
                    /* Keyboard/IME viewport changes can induce a synthetic page
                     * scroll. Ignore that short transition. */
        ...
        DesktopWindowManager.prototype.work_area = function() {
            /* The taskbar intentionally floats above windows.  Keeping the full
             * viewport here lets a window be dragged/resized behind the taskbar
             * while its title bar remains constrained to the visible screen. */
            return {
        ...
        DesktopWindowManager.prototype.min_width = function(record, max_width) {
            /* Desktop dialogs can keep their preferred minimum.  On a touch
             * viewport the minimum must be independent of desktop minWidth so
             * the lower-right handle can also reduce the window width. */
        ...
        DesktopWindowManager.prototype.clamp_rect = function(record, rect) {
            ...
            /* The left frame edge may leave the screen.  Keep only a small
             * title-bar strip visible so the window remains recoverable. */
        ...
        DesktopWindowManager.prototype.persist_size = function(record) {
            ...
            this.window_size_save_timer = setTimeout(function() {
                ...
                HF.save_preferences({
                    ...
                    /* A transient UCI write failure should not interrupt resizing. */
                });
            }, 420);
        };
        ...
        DesktopWindowManager.prototype.initial_rect = function(record) {
            ...
            var taskbar_top = this.taskbar_top(area);
            /* Documents open centred rather than cascading into a corner.  The
             * centre uses the space above the taskbar for a natural first view. */
            return {
        ...
        DesktopWindowManager.prototype.focus = function(record) {
            ...
            /* A modal window floats above the modal overlay and stays on top. */
            if (record.modal) {
                this.active = record;
            ...
            /* While a modal overlay is active, focusing another window only
             * re-raises the modal window; the user must dismiss it first. */
        ...
        DesktopWindowManager.prototype.minimize = function(record) {
            ...
            this.render_taskbar();
            /* A freshly minimized window is discoverable for one short interval
             * before the normal auto-hide timer returns the taskbar off-screen. */
            this.set_taskbar_revealed(true);
        };
        ...
        DesktopWindowManager.prototype.restore = function(record) {
            ...
            if (record.maximized) {
                ...
                if (saved_viewport && saved_viewport.width === window.innerWidth &&
                    ...
                    /* A taskbar restore returns precisely to the location where
                     * this window was minimized, including a negative left edge. */
        ...
        DesktopWindowManager.prototype.close = function(record) {
            ...
            if (record.before_close) {
                ...
                if (result && typeof result.then === 'function') {
                    /* Async confirmation (window-based): wait for the promise. */
        ...
        DesktopWindowManager.prototype.begin_interaction = function(record, event, kind, edge) {
            ...
            if (original_maximized && kind === 'drag') {
                /* Dragging a maximized desktop window restores it under the pointer. */
                this.restore_size(record);
        ...
        DesktopWindowManager.prototype.open_legacy = function(element, display_mode) {
            ...
            record.display_mode = display_mode || record.display_mode || 'block';
            record.closed = false;
            record.minimized = false;
    ...
    // ---- public WindowManager API ----
    HF.WindowManager = HF.window_manager;
    HF.WindowManager.openLegacy = HF.window_manager.open_legacy;   // camelCase alias
