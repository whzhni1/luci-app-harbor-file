...
function allow_system_operations() {
	let uci = cursor();
	uci.load('harbor_file');
	let v = uci.get('harbor_file', 'main', 'allow_system_operations');
	uci.unload('harbor_file');
	return (v == '1');
}
...
function home_dir() {
	let uci = cursor();
	uci.load('harbor_file');
	let v = uci.get('harbor_file', 'main', 'home_dir');
	uci.unload('harbor_file');
...
function main() {
	...
	if (reason) {
		warn(sprintf('[harbor-io] 403 mode=%s path=%s sid=%s: %s\n',
